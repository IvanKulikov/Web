...
function articles_get($link, $id_article){
  ...
  if(!$result) die (mysqli_erгоr($link));
...
if ($title == '' )
return false;
$sql = "UPDATE articles SET title='%s', content='%s', data='%s' WHERE
id='%d'";
$query = sprintf($sql, mysqli_real_escape_string($link, $title),
mysqli_real_escape_string($link, $content), mysqli_real_escape_string($link,
$data), $id);
$result = mysqli_query($link, $query);
if (!$result)
...
if ($title == '')
return false;
$t = "INSERT INTO articles (title, date, content) VALUES ('%s', '%s',
'%s')";
